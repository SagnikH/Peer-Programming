...
class AutomergeStore {
	...
	get(docId) {
		if (this.has(docId)) return this.#store.get(docId);
		return null;
	}
...
function SessionManager(io, dbManager) {
	...
	function log(...data) {
		//console.log("[Session Manager]", data); }
		...
		async function docInit(socket, docId) {
			...
			if (changes === null) {
				...
				socket
					.to(socket.sessionId)
					.emit(CLIENT_OPENED_DOC, { clientId: socket.id, docId });
				socket.join(docId);
			}
		...
		async function docClosed(socket, docId) {
			...
			if (sockets.size === 0) {
				...
				socket
					.to(socket.sessionId)
					.emit(CLIENT_CLOSED_DOC, { clientId: socket.id, docId });
			}
		}
		...
		io.on("connection", async (socket) => {
			...
			socket.on("disconnect", async (reason) => {
				log(
					`${socket.userId} disconnected | session: ${socket.sessionId}`,
					reason
				);
				if (socket.docId) await docClosed(socket, socket.docId);
				socket
					.to(socket.sessionId)
					.emit(CLIENT_DISCONNECTED, { clientId: socket.id });
			});
			...
			socket
				.to(socket.sessionId)
				.emit(CLIENT_CONNECTED, { clientId: socket.id });
